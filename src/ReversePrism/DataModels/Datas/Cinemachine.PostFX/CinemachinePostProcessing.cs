...
namespace ReversePrism.DataModels
{
    ...
    // 048 M_Profile                                ModelClassType PostProcessProfile PostProcessProfile PostProcessProfile Pointer
    // 008 sVolumeOwnerName                         string IL2CPP_TYPE_STRING
    // 010 SVolumes                                 ModelClassListType List`1<PostProcessVolume> List`1<PostProcessVolume> List<PostProcessVolume> Pointer
    // 018 mBrainToLayer                            Dictionary`2<CinemachineBrain, PostProcessLayer> IL2CPP_TYPE_GENERICINST
    public partial class CinemachinePostProcessing : DataModel
    {
        ...
        public PostProcessProfile?                      M_Profile                               { get; set; }
        public List<PostProcessVolume>?                 SVolumes                                { get; set; }

        public static CinemachinePostProcessing? FromPointer(IntPtr p0)
        {
            if(p0 == IntPtr.Zero)
                return null;

            var p       = p0.ToInt64();
            var value   = new CinemachinePostProcessing() { Pointer= p0 };
            ...
            value.M_Profile                                 = GetObject<PostProcessProfile>(new IntPtr(p + 0x048), ReversePrism.DataModels.PostProcessProfile.FromPointer); // 0x48 M_Profile                   ( ModelClassType PostProcessProfile PostProcessProfile PostProcessProfile Pointer )
            value.SVolumes                                  = GetObjectList<PostProcessVolume>(new IntPtr(p + 0x010), ReversePrism.DataModels.PostProcessVolume.FromPointer); // 0x10 SVolumes                    ( ModelClassListType List`1<PostProcessVolume> List`1<PostProcessVolume> List<PostProcessVolume> Pointer )
