...
namespace ReversePrism.DataModels
{
    ...
    public partial class SslClientAuthenticationOptions
    {
        ...
        public static SslClientAuthenticationOptions? FromPointer(IntPtr p0)
        {
            ...
            var p       = p0.ToInt64();
            var value   = new SslClientAuthenticationOptions();

            value.EncryptionPolicy                          = (EncryptionPolicy)GetInt32(new IntPtr(p + 0x010)); // 0270D7928530 0x10 EncryptionPolicy            ( 00018672FD10 ModelEnumType EncryptionPolicy EncryptionPolicy EncryptionPolicy Int32 )
            value.CheckCertificateRevocation                = (X509RevocationMode)GetInt32(new IntPtr(p + 0x014)); // 0270D7928550 0x14 CheckCertificateRevocation  ( 00018656F3E0 ModelEnumType X509RevocationMode X509RevocationMode X509RevocationMode Int32 )
            value.EnabledSslProtocols                       = (SslProtocols)GetInt32(new IntPtr(p + 0x018)); // 0270D7928570 0x18 EnabledSslProtocols         ( 00018655D2F0 ModelEnumType SslProtocols SslProtocols SslProtocols Int32 )
            value.AllowRenegotiation                        = GetBool(new IntPtr(p + 0x01C)); // 0270D7928590 0x1C AllowRenegotiation          ( 000186594D10 ModelPrimitiveType bool bool bool Bool )
            value.LocalCertificateSelectionCallback         = GetObject<LocalCertificateSelectionCallback>(new IntPtr(p + 0x020), ReversePrism.DataModels.LocalCertificateSelectionCallback.FromPointer); // 0270D79285B0 0x20 LocalCertificateSelectionCallback ( 0001865A2C80 ModelClassType LocalCertificateSelectionCallback LocalCertificateSelectionCallback LocalCertificateSelectionCallback Pointer )
            value.TargetHost                                = GetString(new IntPtr(p + 0x028)); // 0270D79285D0 0x28 TargetHost                  ( 000186671910 ModelPrimitiveType string string string String )
            value.ClientCertificates                        = GetObject<X509CertificateCollection>(new IntPtr(p + 0x030), ReversePrism.DataModels.X509CertificateCollection.FromPointer); // 0270D79285F0 0x30 ClientCertificates          ( 0001865656A0 ModelClassType X509CertificateCollection X509CertificateCollection X509CertificateCollection Pointer )
