...
namespace ReversePrism.DataModels
{
    ...
    public partial class EpisodeSelectViewBannerGroupPresenter : DataModel
    {
        public IEpisodeSelectViewBannerGroupForLegacy?  View                                    { get; set; }
        public CompositeDisposable?                     Disposable                              { get; set; }

        public static EpisodeSelectViewBannerGroupPresenter? FromPointer(IntPtr p0)
        {
            if(p0 == IntPtr.Zero)
                return null;

            var p       = p0.ToInt64();
            var value   = new EpisodeSelectViewBannerGroupPresenter() { Pointer= p0 };
