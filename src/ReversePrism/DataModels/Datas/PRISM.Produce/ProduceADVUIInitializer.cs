...
namespace ReversePrism.DataModels
{
    ...
    public partial class ProduceADVUIInitializer
    {
        ...
        public static ProduceADVUIInitializer? FromPointer(IntPtr p0)
        {
            ...
            var p       = p0.ToInt64();
            var value   = new ProduceADVUIInitializer();

            value.AdvChoiceButtonGroupViewModel             = GetObject<AdvChoiceButtonGroupViewModel>(new IntPtr(p + 0x010), ReversePrism.DataModels.AdvChoiceButtonGroupViewModel.FromPointer); // 027006606078 0x10 AdvChoiceButtonGroupViewModel ( 000186694210 ModelClassType AdvChoiceButtonGroupViewModel AdvChoiceButtonGroupViewModel AdvChoiceButtonGroupViewModel Pointer )
            value.CurrentPlayableADVView                    = GetObject<PlayableADV>(new IntPtr(p + 0x018), ReversePrism.DataModels.PlayableADV.FromPointer); // 027006606098 0x18 CurrentPlayableADVView      ( 00018674CC30 ModelClassType PlayableADV PlayableADV PlayableADV Pointer )
            value.IsAdv                                     = GetBool(new IntPtr(p + 0x020)); // 0270066060B8 0x20 IsAdv                       ( 000186594D10 ModelPrimitiveType bool bool bool Bool )
