...
namespace ReversePrism.DataModels
{
    ...
    public partial class MD5CryptoServiceProvider : DataModel
    {
        ...
        public static MD5CryptoServiceProvider? FromPointer(IntPtr p0)
        {
            ...
            value.H                                         = GetUInt32List(new IntPtr(p + 0x028)); // 0245A3D05D78 0x28 H                           ( 000185B83830 ModelPrimitiveListType uint[] uint[] List<uint> Pointer )
            value.Buff                                      = GetUInt32List(new IntPtr(p + 0x030)); // 0245A3D05D98 0x30 Buff                        ( 000185B83830 ModelPrimitiveListType uint[] uint[] List<uint> Pointer )
            value.Count                                     = GetUInt64(new IntPtr(p + 0x038)); // 0245A3D05DB8 0x38 Count                       ( 00018669B890 ModelPrimitiveType ulong ulong ulong UInt64 )
            value.ProcessingBuffer                          = GetSByteList(new IntPtr(p + 0x040)); // 0245A3D05DD8 0x40 ProcessingBuffer            ( 000185B79750 ModelPrimitiveListType sbyte[] sbyte[] List<sbyte> Pointer )
            value.ProcessingBufferCount                     = GetInt32(new IntPtr(p + 0x048)); // 0245A3D05DF8 0x48 ProcessingBufferCount       ( 0001865F2AF0 ModelPrimitiveType int int int Int32 )
