...
namespace ReversePrism.DataModels
{
    ...
    public partial class FinishChallengeTourStageReply
    {
        ...
        public static FinishChallengeTourStageReply? FromPointer(IntPtr p0)
        {
            ...
            var p       = p0.ToInt64();
            var value   = new FinishChallengeTourStageReply();

            value.RewardList                                = GetObjectList<ChallengeTourStageRewardStatus>(new IntPtr(p + 0x018), ReversePrism.DataModels.ChallengeTourStageRewardStatus.FromPointer); // 0270D0B3FD48 0x18 RewardList                  ( 000185CD1758 ModelClassListType RepeatedField`1<ChallengeTourStageRewardStatus> RepeatedField`1<ChallengeTourStageRewardStatus> List<ChallengeTourStageRewardStatus> Pointer )
            value.HavingGroup                               = GetObject<HavingProductGroupStatus>(new IntPtr(p + 0x020), ReversePrism.DataModels.HavingProductGroupStatus.FromPointer); // 0270D0B3FD88 0x20 HavingGroup                 ( 0001866ACFE0 ModelClassType HavingProductGroupStatus HavingProductGroupStatus HavingProductGroupStatus Pointer )
            value.Fan                                       = GetInt32(new IntPtr(p + 0x028)); // 0270D0B3FDC8 0x28 Fan                         ( 0001865F2AF0 ModelPrimitiveType int int int Int32 )
            value.ChallengeTourStage                        = GetObject<ChallengeTourStageStatus>(new IntPtr(p + 0x030), ReversePrism.DataModels.ChallengeTourStageStatus.FromPointer); // 0270D0B3FE08 0x30 ChallengeTourStage          ( 000186584860 ModelClassType ChallengeTourStageStatus ChallengeTourStageStatus ChallengeTourStageStatus Pointer )
            value.IdolBaseList                              = GetObjectList<IdolBaseStatus>(new IntPtr(p + 0x038), ReversePrism.DataModels.IdolBaseStatus.FromPointer); // 0270D0B3FE68 0x38 IdolBaseList                ( 000185CDF648 ModelClassListType RepeatedField`1<IdolBaseStatus> RepeatedField`1<IdolBaseStatus> List<IdolBaseStatus> Pointer )
