...
namespace ReversePrism.DataModels
{
    ...
    public partial class PrismStreamingNetwork
    {
        ...
        public static PrismStreamingNetwork? FromPointer(IntPtr p0)
        {
            ...
            var p       = p0.ToInt64();
            var value   = new PrismStreamingNetwork();

            value.InstantiatedGuid                          = GetStringList(new IntPtr(p + 0x010)); // 0270D4F28758 0x10 InstantiatedGuid            ( 000185D0D738 ModelPrimitiveListType List`1<string> List`1<string> List<string> Pointer )
            value.SoundViewUsedForSynchronization           = GetObject<IPrismStreamingCurrentTime>(new IntPtr(p + 0x018), ReversePrism.DataModels.IPrismStreamingCurrentTime.FromPointer); // 0270D4F28778 0x18 SoundViewUsedForSynchronization ( 0001865DE480 ModelClassType IPrismStreamingCurrentTime IPrismStreamingCurrentTime IPrismStreamingCurrentTime Pointer )
            value.Callbacks                                 = GetObjectList<IPrismStreamingCallbacks>(new IntPtr(p + 0x020), ReversePrism.DataModels.IPrismStreamingCallbacks.FromPointer); // 0270D4F28798 0x20 Callbacks                   ( 000185CE6E78 ModelClassListType List`1<IPrismStreamingCallbacks> List`1<IPrismStreamingCallbacks> List<IPrismStreamingCallbacks> Pointer )
