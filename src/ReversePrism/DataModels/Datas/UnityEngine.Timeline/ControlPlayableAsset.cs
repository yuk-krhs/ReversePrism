...
namespace ReversePrism.DataModels
{
    ...
    public partial class ControlPlayableAsset
    {
        ...
        public static ControlPlayableAsset? FromPointer(IntPtr p0)
        {
            ...
            var p       = p0.ToInt64();
            var value   = new ControlPlayableAsset();

            value.PrefabGameObject                          = GetObject<GameObject>(new IntPtr(p + 0x028), ReversePrism.DataModels.GameObject.FromPointer); // 0270DB24D7B0 0x28 PrefabGameObject            ( 0001865D8420 ModelClassType GameObject GameObject GameObject Pointer )
            value.UpdateParticle                            = GetBool(new IntPtr(p + 0x030)); // 0270DB24D7D0 0x30 UpdateParticle              ( 000186595960 ModelPrimitiveType bool bool bool Bool )
            value.ParticleRandomSeed                        = GetUInt32(new IntPtr(p + 0x034)); // 0270DB24D7F0 0x34 ParticleRandomSeed          ( 0001866992B0 ModelPrimitiveType uint uint uint UInt32 )
            value.UpdateDirector                            = GetBool(new IntPtr(p + 0x038)); // 0270DB24D810 0x38 UpdateDirector              ( 000186595960 ModelPrimitiveType bool bool bool Bool )
            value.UpdateITimeControl                        = GetBool(new IntPtr(p + 0x039)); // 0270DB24D830 0x39 UpdateITimeControl          ( 000186595960 ModelPrimitiveType bool bool bool Bool )
            value.SearchHierarchy                           = GetBool(new IntPtr(p + 0x03A)); // 0270DB24D850 0x3A SearchHierarchy             ( 000186595960 ModelPrimitiveType bool bool bool Bool )
            value.Active                                    = GetBool(new IntPtr(p + 0x03B)); // 0270DB24D870 0x3B Active                      ( 000186595960 ModelPrimitiveType bool bool bool Bool )
            value.PostPlayback                              = (PostPlaybackState)GetInt32(new IntPtr(p + 0x03C)); // 0270DB24D890 0x3C PostPlayback                ( 000186689970 ModelEnumType PostPlaybackState PostPlaybackState PostPlaybackState Int32 )
            value.M_ControlDirectorAsset                    = GetObject<PlayableAsset>(new IntPtr(p + 0x040), ReversePrism.DataModels.PlayableAsset.FromPointer); // 0270DB24D8B0 0x40 M_ControlDirectorAsset      ( 00018674D0F0 ModelClassType PlayableAsset PlayableAsset PlayableAsset Pointer )
            value.M_Duration                                = GetDouble(new IntPtr(p + 0x048)); // 0270DB24D8D0 0x48 M_Duration                  ( 0001865C2950 ModelPrimitiveType double double double Double )
            value.M_SupportLoop                             = GetBool(new IntPtr(p + 0x050)); // 0270DB24D8F0 0x50 M_SupportLoop               ( 000186594D10 ModelPrimitiveType bool bool bool Bool )
            value.ControllingDirectors                      = GetBool(new IntPtr(p + 0x051)); // 0270DB24D950 0x51 ControllingDirectors        ( 000186594D10 ModelPrimitiveType bool bool bool Bool )
            value.ControllingParticles                      = GetBool(new IntPtr(p + 0x052)); // 0270DB24D970 0x52 ControllingParticles        ( 000186594D10 ModelPrimitiveType bool bool bool Bool )
