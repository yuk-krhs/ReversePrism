...
namespace ReversePrism.DataModels
{
    ...
    public partial class TimelinePlayable
    {
        ...
        public static TimelinePlayable? FromPointer(IntPtr p0)
        {
            ...
            var p       = p0.ToInt64();
            var value   = new TimelinePlayable();

            value.M_ActiveClips                             = GetObjectList<RuntimeElement>(new IntPtr(p + 0x018), ReversePrism.DataModels.RuntimeElement.FromPointer); // 0270072DCD60 0x18 M_ActiveClips               ( 000185D048E8 ModelClassListType List`1<RuntimeElement> List`1<RuntimeElement> List<RuntimeElement> Pointer )
            value.M_CurrentListOfActiveClips                = GetObjectList<RuntimeElement>(new IntPtr(p + 0x020), ReversePrism.DataModels.RuntimeElement.FromPointer); // 0270072DCD80 0x20 M_CurrentListOfActiveClips  ( 000185D048E8 ModelClassListType List`1<RuntimeElement> List`1<RuntimeElement> List<RuntimeElement> Pointer )
            value.M_ActiveBit                               = GetInt32(new IntPtr(p + 0x028)); // 0270072DCDA0 0x28 M_ActiveBit                 ( 0001865F2AF0 ModelPrimitiveType int int int Int32 )
            value.M_EvaluateCallbacks                       = GetObjectList<ITimelineEvaluateCallback>(new IntPtr(p + 0x030), ReversePrism.DataModels.ITimelineEvaluateCallback.FromPointer); // 0270072DCDC0 0x30 M_EvaluateCallbacks         ( 000185CE95E8 ModelClassListType List`1<ITimelineEvaluateCallback> List`1<ITimelineEvaluateCallback> List<ITimelineEvaluateCallback> Pointer )
