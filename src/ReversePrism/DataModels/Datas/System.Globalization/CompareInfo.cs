...
namespace ReversePrism.DataModels
{
    ...
    public partial class CompareInfo
    {
        ...
        public static CompareInfo? FromPointer(IntPtr p0)
        {
            ...
            var p       = p0.ToInt64();
            var value   = new CompareInfo();

            value.M_name                                    = GetString(new IntPtr(p + 0x010)); // 0270001D0B98 0x10 M_name                      ( 000186671910 ModelPrimitiveType string string string String )
            value.SortName                                  = GetString(new IntPtr(p + 0x018)); // 0270001D0BB8 0x18 SortName                    ( 0001866742E0 ModelPrimitiveType string string string String )
            value.M_SortVersion                             = GetObject<SortVersion>(new IntPtr(p + 0x020), ReversePrism.DataModels.SortVersion.FromPointer); // 0270001D0BD8 0x20 M_SortVersion               ( 000186532440 ModelClassType SortVersion SortVersion SortVersion Pointer )
            value.Culture                                   = GetInt32(new IntPtr(p + 0x028)); // 0270001D0BF8 0x28 Culture                     ( 0001865F2AF0 ModelPrimitiveType int int int Int32 )
            value.Collator                                  = GetObject<ISimpleCollator>(new IntPtr(p + 0x030), ReversePrism.DataModels.ISimpleCollator.FromPointer); // 0270001D0C18 0x30 Collator                    ( 000186645C00 ModelClassType ISimpleCollator ISimpleCollator ISimpleCollator Pointer )
            value.ManagedCollation                          = GetBool(new IntPtr(p + 0x010)); // 0270001D0C58 0x10 ManagedCollation            ( 000186595C30 ModelPrimitiveType bool bool bool Bool )
            value.ManagedCollationChecked                   = GetBool(new IntPtr(p + 0x011)); // 0270001D0C78 0x11 ManagedCollationChecked     ( 000186595C30 ModelPrimitiveType bool bool bool Bool )
