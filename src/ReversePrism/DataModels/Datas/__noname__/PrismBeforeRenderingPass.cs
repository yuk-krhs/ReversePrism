...
namespace ReversePrism.DataModels
{
    ...
    public partial class PrismBeforeRenderingPass : DataModel
    {
        public PrismRenderPipelineAsset?                Asset                                   { get; set; }
        public PrismRenderFrame?                        PrismRenderFrame                        { get; set; }

        public static PrismBeforeRenderingPass? FromPointer(IntPtr p0)
        {
            if(p0 == IntPtr.Zero)
                return null;

            var p       = p0.ToInt64();
            var value   = new PrismBeforeRenderingPass() { Pointer= p0 };
