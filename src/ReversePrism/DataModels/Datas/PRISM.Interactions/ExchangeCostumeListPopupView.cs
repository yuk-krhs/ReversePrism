...
namespace ReversePrism.DataModels
{
    ...
    public partial class ExchangeCostumeListPopupView : DataModel
    {
        ...
        public static ExchangeCostumeListPopupView? FromPointer(IntPtr p0)
        {
            ...
            var p       = p0.ToInt64();
            var value   = new ExchangeCostumeListPopupView() { Pointer= p0 };

            value.CellPrefab                                = GetObject<GameObject>(new IntPtr(p + 0x020), ReversePrism.DataModels.GameObject.FromPointer); // 02466B969578 0x20 CellPrefab                  ( 0001865D7D30 ModelClassType GameObject GameObject GameObject Pointer )
            value.Content                                   = GetObject<Transform>(new IntPtr(p + 0x028), ReversePrism.DataModels.Transform.FromPointer); // 02466B969598 0x28 Content                     ( 0001866AA150 ModelClassType Transform Transform Transform Pointer )
            value.ViewModel                                 = GetObject<ExchangeSelectionListPopupViewModel>(new IntPtr(p + 0x030), ReversePrism.DataModels.ExchangeSelectionListPopupViewModel.FromPointer); // 02466B9695B8 0x30 ViewModel                   ( 00018651A2F0 ModelClassType ExchangeSelectionListPopupViewModel ExchangeSelectionListPopupViewModel ExchangeSelectionListPopupViewModel Pointer )
            value.Parent                                    = GetObject<IPopupFrameView>(new IntPtr(p + 0x050), ReversePrism.DataModels.IPopupFrameView.FromPointer); // 02466B969638 0x50 Parent                      ( 0001865DAC10 ModelClassType IPopupFrameView IPopupFrameView IPopupFrameView Pointer )
