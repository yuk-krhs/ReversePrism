...
namespace ReversePrism.DataModels
{
    ...
    public partial class UnitySynchronizationContext
    {
        ...
        public static UnitySynchronizationContext? FromPointer(IntPtr p0)
        {
            ...
            value.M_AsyncWorkQueue                          = GetEnumList<WorkRequest>(new IntPtr(p + 0x018)); // 02700206B698 0x18 M_AsyncWorkQueue            ( 000185D2FF38 ModelEnumListType List`1<WorkRequest> List`1<WorkRequest> List<WorkRequest> Pointer )
            value.M_CurrentFrameWork                        = GetEnumList<WorkRequest>(new IntPtr(p + 0x020)); // 02700206B6B8 0x20 M_CurrentFrameWork          ( 000185D2FF38 ModelEnumListType List`1<WorkRequest> List`1<WorkRequest> List<WorkRequest> Pointer )
            value.M_MainThreadID                            = GetInt32(new IntPtr(p + 0x028)); // 02700206B6D8 0x28 M_MainThreadID              ( 0001865F4260 ModelPrimitiveType int int int Int32 )
            value.M_TrackedCount                            = GetInt32(new IntPtr(p + 0x02C)); // 02700206B6F8 0x2C M_TrackedCount              ( 0001865F2AF0 ModelPrimitiveType int int int Int32 )
