...
namespace ReversePrism.DataModels
{
    ...
    public partial class RivalUnitStatus
    {
        ...
        public static RivalUnitStatus? FromPointer(IntPtr p0)
        {
            ...
            var p       = p0.ToInt64();
            var value   = new RivalUnitStatus();

            value.RivalList                                 = GetObjectList<RivalStatus>(new IntPtr(p + 0x018), ReversePrism.DataModels.RivalStatus.FromPointer); // 0270D1136120 0x18 RivalList                   ( 000185CEF118 ModelClassListType RepeatedField`1<RivalStatus> RepeatedField`1<RivalStatus> List<RivalStatus> Pointer )
            value.ProduceCardDeck                           = GetObject<ProduceCardDeckStatus>(new IntPtr(p + 0x020), ReversePrism.DataModels.ProduceCardDeckStatus.FromPointer); // 0270D1136160 0x20 ProduceCardDeck             ( 000186536800 ModelClassType ProduceCardDeckStatus ProduceCardDeckStatus ProduceCardDeckStatus Pointer )
            value.AppealCorrection                          = GetObject<AppealCorrectionStatus>(new IntPtr(p + 0x028), ReversePrism.DataModels.AppealCorrectionStatus.FromPointer); // 0270D11361A0 0x28 AppealCorrection            ( 0001866CC8F0 ModelClassType AppealCorrectionStatus AppealCorrectionStatus AppealCorrectionStatus Pointer )
