...
namespace ReversePrism.DataModels
{
    ...
    public partial class LocalDataStoreMgr
    {
        ...
        public static LocalDataStoreMgr? FromPointer(IntPtr p0)
        {
            ...
            var p       = p0.ToInt64();
            var value   = new LocalDataStoreMgr();

            value.M_SlotInfoTable                           = GetBoolList(new IntPtr(p + 0x010)); // 027001532360 0x10 M_SlotInfoTable             ( 000185B78CA0 ModelPrimitiveListType bool[] bool[] List<bool> Pointer )
            value.M_FirstAvailableSlot                      = GetInt32(new IntPtr(p + 0x018)); // 027001532380 0x18 M_FirstAvailableSlot        ( 0001865F2AF0 ModelPrimitiveType int int int Int32 )
            value.M_ManagedLocalDataStores                  = GetObjectList<LocalDataStore>(new IntPtr(p + 0x020), ReversePrism.DataModels.LocalDataStore.FromPointer); // 0270015323A0 0x20 M_ManagedLocalDataStores    ( 000185CF0B98 ModelClassListType List`1<LocalDataStore> List`1<LocalDataStore> List<LocalDataStore> Pointer )
            value.M_CookieGenerator                         = GetInt64(new IntPtr(p + 0x030)); // 0270015323E0 0x30 M_CookieGenerator           ( 0001865F7700 ModelPrimitiveType long long long Int64 )
