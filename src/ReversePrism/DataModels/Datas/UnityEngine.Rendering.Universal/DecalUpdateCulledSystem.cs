...
namespace ReversePrism.DataModels
{
    ...
    public partial class DecalUpdateCulledSystem : DataModel
    {
        public DecalEntityManager?                      M_EntityManager                         { get; set; }
        public ProfilingSampler?                        M_Sampler                               { get; set; }

        public static DecalUpdateCulledSystem? FromPointer(IntPtr p0)
        {
            if(p0 == IntPtr.Zero)
                return null;

            var p       = p0.ToInt64();
            var value   = new DecalUpdateCulledSystem() { Pointer= p0 };
