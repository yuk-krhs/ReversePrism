...
namespace ReversePrism.DataModels
{
    ...
    // 020 M_MaxDrawDistance                        ModelPrimitiveType float float float Single
    public partial class DecalCreateDrawCallSystem : DataModel
    {
        public DecalEntityManager?                      M_EntityManager                         { get; set; }
        public ProfilingSampler?                        M_Sampler                               { get; set; }
        public float                                    M_MaxDrawDistance                       { get; set; }

        public static DecalCreateDrawCallSystem? FromPointer(IntPtr p0)
        {
            if(p0 == IntPtr.Zero)
                return null;

            var p       = p0.ToInt64();
            var value   = new DecalCreateDrawCallSystem() { Pointer= p0 };
            ...
            value.M_MaxDrawDistance                         = GetSingle(new IntPtr(p + 0x020)); // 0x20 M_MaxDrawDistance           ( ModelPrimitiveType float float float Single )
