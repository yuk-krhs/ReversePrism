...
namespace ReversePrism.DataModels
{
    ...
    public partial class TelemetryMetricsInstanceWrapper : DataModel
    {
        ...
        public static TelemetryMetricsInstanceWrapper? FromPointer(IntPtr p0)
        {
            ...
            value.M_Instance                                = GetObject<IMetrics>(new IntPtr(p + 0x010), ReversePrism.DataModels.IMetrics.FromPointer); // 0245A68F4218 0x10 M_Instance                  ( 0001865B2F70 ModelClassType IMetrics IMetrics IMetrics Pointer )
            value.M_Logger                                  = GetObject<ILogger>(new IntPtr(p + 0x018), ReversePrism.DataModels.ILogger.FromPointer); // 0245A68F4238 0x18 M_Logger                    ( 0001865A1000 ModelClassType ILogger ILogger ILogger Pointer )
            value.M_Util                                    = GetObject<IUtil>(new IntPtr(p + 0x020), ReversePrism.DataModels.IUtil.FromPointer); // 0245A68F4258 0x20 M_Util                      ( 0001866900E0 ModelClassType IUtil IUtil IUtil Pointer )
