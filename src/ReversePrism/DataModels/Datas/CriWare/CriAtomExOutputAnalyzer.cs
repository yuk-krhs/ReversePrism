...
namespace ReversePrism.DataModels
{
    ...
    public partial class CriAtomExOutputAnalyzer
    {
        ...
        public static CriAtomExOutputAnalyzer? FromPointer(IntPtr p0)
        {
            ...
            var p       = p0.ToInt64();
            var value   = new CriAtomExOutputAnalyzer();

            value.Player                                    = GetObject<CriAtomExPlayer>(new IntPtr(p + 0x028), ReversePrism.DataModels.CriAtomExPlayer.FromPointer); // 0270D1881358 0x28 Player                      ( 0001866490B0 ModelClassType CriAtomExPlayer CriAtomExPlayer CriAtomExPlayer Pointer )
            value.BusName                                   = GetString(new IntPtr(p + 0x030)); // 0270D1881378 0x30 BusName                     ( 000186671E00 ModelPrimitiveType string string string String )
            value.NumBands                                  = GetInt32(new IntPtr(p + 0x038)); // 0270D1881398 0x38 NumBands                    ( 0001865F3220 ModelPrimitiveType int int int Int32 )
            value.NumCapturedPcmSamples                     = GetInt32(new IntPtr(p + 0x03C)); // 0270D18813B8 0x3C NumCapturedPcmSamples       ( 0001865F3220 ModelPrimitiveType int int int Int32 )
            value.UserPcmCaptureCallback                    = GetObject<PcmCaptureCallback>(new IntPtr(p + 0x020), ReversePrism.DataModels.PcmCaptureCallback.FromPointer); // 0270D18814D8 0x20 UserPcmCaptureCallback      ( 00018654E790 ModelClassType PcmCaptureCallback PcmCaptureCallback PcmCaptureCallback Pointer )
            value.DataL                                     = GetSingleList(new IntPtr(p + 0x010)); // 0270D1881498 0x10 DataL                       ( 000185B80B40 ModelPrimitiveListType float[] float[] List<float> Pointer )
            value.DataR                                     = GetSingleList(new IntPtr(p + 0x018)); // 0270D18814B8 0x18 DataR                       ( 000185B80B40 ModelPrimitiveListType float[] float[] List<float> Pointer )
