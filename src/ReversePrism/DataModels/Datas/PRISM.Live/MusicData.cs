...
namespace ReversePrism.DataModels
{
    ...
    public partial class MusicData
    {
        ...
        public static MusicData? FromPointer(IntPtr p0)
        {
            ...
            var p       = p0.ToInt64();
            var value   = new MusicData();

            value.Song                                      = GetObject<ISongStatus>(new IntPtr(p + 0x028), ReversePrism.DataModels.ISongStatus.FromPointer); // 0270015F8560 0x28 Song                        ( 000186649210 ModelClassType ISongStatus ISongStatus ISongStatus Pointer )
            value.Master                                    = GetObject<MstSong>(new IntPtr(p + 0x030), ReversePrism.DataModels.MstSong.FromPointer); // 0270015F8580 0x30 Master                      ( 00018662C3A0 ModelClassType MstSong MstSong MstSong Pointer )
            value.IsNew                                     = GetBool(new IntPtr(p + 0x038)); // 0270015F85A0 0x38 IsNew                       ( 000186594D10 ModelPrimitiveType bool bool bool Bool )
            value.IsOnceDisplayed                           = GetBool(new IntPtr(p + 0x039)); // 0270015F85C0 0x39 IsOnceDisplayed             ( 000186594D10 ModelPrimitiveType bool bool bool Bool )
            value.IsOnceClearedAnyDifficulty                = GetBool(new IntPtr(p + 0x03A)); // 0270015F85E0 0x3A IsOnceClearedAnyDifficulty  ( 0001865965D0 ModelPrimitiveType bool bool bool Bool )
