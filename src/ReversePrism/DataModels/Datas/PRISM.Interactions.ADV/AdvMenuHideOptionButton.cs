...
namespace ReversePrism.DataModels
{
    ...
    public partial class AdvMenuHideOptionButton : DataModel
    {
        public UIButton?                                Button                                  { get; set; }
        public UITextMeshProUGUI?                       ButtonText                              { get; set; }

        public static AdvMenuHideOptionButton? FromPointer(IntPtr p0)
        {
            if(p0 == IntPtr.Zero)
                return null;

            var p       = p0.ToInt64();
            var value   = new AdvMenuHideOptionButton() { Pointer= p0 };
