...
namespace ReversePrism.DataModels
{
    ...
    public partial class AdvMenuShowLogOptionButton : DataModel
    {
        public UIButton?                                Button                                  { get; set; }
        public UITextMeshProUGUI?                       ButtonText                              { get; set; }

        public static AdvMenuShowLogOptionButton? FromPointer(IntPtr p0)
        {
            if(p0 == IntPtr.Zero)
                return null;

            var p       = p0.ToInt64();
            var value   = new AdvMenuShowLogOptionButton() { Pointer= p0 };
