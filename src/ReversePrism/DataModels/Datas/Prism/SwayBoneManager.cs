...
namespace ReversePrism.DataModels
{
    ...
    public partial class SwayBoneManager
    {
        ...
        public static SwayBoneManager? FromPointer(IntPtr p0)
        {
            ...
            var p       = p0.ToInt64();
            var value   = new SwayBoneManager();

            value.WindVector                                = (Vector3)GetInt32(new IntPtr(p + 0x010)); // 02700359DDA8 0x10 WindVector                  ( 0001866AC1F0 ModelEnumType Vector3 Vector3 Vector3 Int32 )
            value.TimeRate                                  = GetSingle(new IntPtr(p + 0x01C)); // 02700359DDC8 0x1C TimeRate                    ( 000186666A20 ModelPrimitiveType float float float Single )
            value.SetDeltaTime                              = GetSingle(new IntPtr(p + 0x020)); // 02700359DDE8 0x20 SetDeltaTime                ( 000186666300 ModelPrimitiveType float float float Single )
            value.DeltaTime                                 = GetSingle(new IntPtr(p + 0x020)); // 02700359DE08 0x20 DeltaTime                   ( 0001866656B0 ModelPrimitiveType float float float Single )
            value.SkipFrame                                 = GetBool(new IntPtr(p + 0x024)); // 02700359DE28 0x24 SkipFrame                   ( 000186594D10 ModelPrimitiveType bool bool bool Bool )
            value.SwayString                                = GetObjectList<SwayString>(new IntPtr(p + 0x028), ReversePrism.DataModels.SwayString.FromPointer); // 02700359DE48 0x28 SwayString                  ( 000185D11578 ModelClassListType List`1<SwayString> List`1<SwayString> List<SwayString> Pointer )
            value.StringNo                                  = GetInt32(new IntPtr(p + 0x028)); // 02700359DE88 0x28 StringNo                    ( 0001865F36C0 ModelPrimitiveType int int int Int32 )
            value.ChangeFlag                                = GetBool(new IntPtr(p + 0x038)); // 02700359DEA8 0x38 ChangeFlag                  ( 000186595C30 ModelPrimitiveType bool bool bool Bool )
            value.ParamChangeFlag                           = GetBool(new IntPtr(p + 0x039)); // 02700359DEC8 0x39 ParamChangeFlag             ( 000186596380 ModelPrimitiveType bool bool bool Bool )
            value.AllocFlag                                 = GetBool(new IntPtr(p + 0x03A)); // 02700359DEE8 0x3A AllocFlag                   ( 000186595C30 ModelPrimitiveType bool bool bool Bool )
            value.Transforms                                = (TransformAccessArray)GetInt32(new IntPtr(p + 0x090)); // 02700359DFC8 0x90 Transforms                  ( 000186690290 ModelEnumType TransformAccessArray TransformAccessArray TransformAccessArray Int32 )
            value.TransformList                             = GetObjectList<Transform>(new IntPtr(p + 0x098), ReversePrism.DataModels.Transform.FromPointer); // 02700359DFE8 0x98 TransformList               ( 000185D160B8 ModelClassListType List`1<Transform> List`1<Transform> List<Transform> Pointer )
